    ...
    def build_memory_data(
        ...
        *,
        ...
        raw_activation_checkpointing = str(
            getattr(tc, "activation_checkpointing", "none") or "none"
        ).lower()
        ...
        if use_moe:
            ...
            if getattr(tc, "attention_type", "mha") == "mla":
                ...
                attention_params = (
                    (tc.hidden_dim * (q_size + 2 * kv_size)) + (q_size * tc.hidden_dim)
                ) / float(max(1, tp))
            ...
            params_per_layer_moe = (
                attention_params + expert_params_per_rank + router_params_per_rank
            )
        ...
        def _build_activation_maps(
            ...
            if activation_checkpointing == "full":
                ...
                recompute_transient_bytes_by_kind[MemKind.TRANSFORMER] = float(
                    transformer_act_layer_local
                )
                ...
                transient_bytes_by_kind[MemKind.MLP] = float(ffn1_bytes)
                recompute_transient_bytes_by_kind[MemKind.LAYERNORM1] = float(
                    base_outputs[MemKind.LAYERNORM1] or 0.0
                )
                recompute_transient_bytes_by_kind[MemKind.LAYERNORM2] = float(
                    base_outputs[MemKind.LAYERNORM2] or 0.0
                )
                ...
                recompute_transient_bytes_by_kind[MemKind.TRANSFORMER] = (
                    selective_transient_transformer
                )
                if attn_score_bytes:
                    transient_bytes_by_kind[MemKind.ATTENTION] = float(attn_score_bytes)
                    recompute_transient_bytes_by_kind[MemKind.ATTENTION] = float(
                        attn_score_bytes
                    )
            else:
                for kind, bytes_val in base_outputs.items():

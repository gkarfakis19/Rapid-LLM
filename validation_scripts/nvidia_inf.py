...
# H100 counterparts (generated by validation_scripts/run_genz_h100.py with the
# same GenZ methodology/system efficiency used for the A100 CSVs).
GENZ_IMEC_H100 = IMEC_DATA_DIR / "H100_inf_genz.csv"
GENZ_NVIDIA_H100 = NVIDIA_DATA_DIR / "4xH100_fp16_Llama3_3-70B_genz.csv"
...
def _plot_combined_ratio_grids_h100(
    ...
    """H100 ratio grid: top row = IMEC Llama2 TP sweeps, bottom row = NVIDIA NIM
    Llama3-70B batch panels. Same style/geometry as the A100 grid, with optional
    baseline-tool series (currently GenZ)."""
    imec_rows = [row for row in imec_rows if row.get("device") == "H100"]
    nvidia_rows = [row for row in nvidia_rows if row.get("device") == "H100"]
    if genz_rows:
        genz_rows = [row for row in genz_rows if row.get("device") == "H100"]
    if genz_nvidia_rows:
        genz_nvidia_rows = [row for row in genz_nvidia_rows if row.get("device") == "H100"]
    if not imec_rows or not nvidia_rows:
        return None
    ...
    for row in imec_rows:
        key = (row.get("model"), int(row.get("tp")))
        actual = row.get("actual_inference_time_s")
        imec_actual[key] = float(actual) if actual is not None else float("nan")
        imec_ratio[key] = _safe_ratio(row.get("inference_time_s"), actual)
    nvidia_actual: Dict[Tuple[int, int, int], float] = {}
    ...
    for row in nvidia_rows:
        ...
        actual = row.get("actual_inference_time_s")
        nvidia_actual[key] = float(actual) if actual is not None else float("nan")
        nvidia_ratio[key] = _safe_ratio(row.get("inference_time_s"), actual)

    # Baseline-tool series, mirroring the A100 grid wiring.
    tool_order = [RAPID_HIER_LABEL, "GenZ"]
    imec_series: Dict[str, Dict[Tuple[object, object], float]] = {RAPID_HIER_LABEL: imec_ratio}
    nvidia_series: Dict[str, Dict[Tuple[int, int, int], float]] = {RAPID_HIER_LABEL: nvidia_ratio}

    if genz_rows:
        genz_lookup = {
            (row.get("model"), int(row.get("tp"))): float(row.get("total_latency", float("nan")))
            for row in genz_rows
        }
        imec_series["GenZ"] = {
            key: _safe_ratio(pred, imec_actual.get(key)) for key, pred in genz_lookup.items()
        }
    if genz_nvidia_rows:
        genz_lookup = {
            (
        ...
        nvidia_series["GenZ"] = {
            key: _safe_ratio(pred, nvidia_actual.get(key)) for key, pred in genz_lookup.items()
        }

    tool_names = []
    for name in tool_order:
        has_vals = any(
            math.isfinite(v) for v in imec_series.get(name, {}).values()
        ) or any(math.isfinite(v) for v in nvidia_series.get(name, {}).values())
        if has_vals:
            tool_names.append(name)
    if not tool_names:
        return None
    ...
    left_vals = [1.0]
    for name in tool_names:
        left_vals.extend(_finite(imec_values.get(name, {}).values()))
    ...
    right_vals = [1.0]
    for name in tool_names:
        right_vals.extend(_finite(nvidia_values.get(name, {}).values()))
    right_span = max(max(right_vals) - min(right_vals), 0.1)
    right_pad = 0.05 * right_span
    right_limits = (0.0, max(right_vals) + right_pad)

    def _draw_missing(ax, pos: float, limits: Tuple[float, float], width: float) -> None:
        ax.bar(
            pos,
            1.0,
            width,
        ...
        # Keep the marker inside the hatched placeholder bar (matches the A100 grid).
        ax.text(pos, 0.5, "x", ha="center", va="center", fontsize=10, color="#555555")

    def _draw_group(ax, base_pos: float, heights: Sequence[float], limits) -> None:
        """Draw one x-group: a bar per tool, or a single wide hatched marker when
        every tool is missing that case."""
        finite_mask = [
            isinstance(h, (int, float)) and math.isfinite(h) for h in heights
        ]
        if not any(finite_mask):
            _draw_missing(ax, base_pos, limits, 0.6)
            return
        for tool_idx, height in enumerate(heights):
            pos = base_pos + offsets[tool_idx]
            if finite_mask[tool_idx]:
                ax.bar(pos, height, bar_width, color=tool_colors[tool_names[tool_idx]])
            else:
                _draw_missing(ax, pos, limits, bar_width)
    ...
    for col_idx, model in enumerate(models):
        ...
        for pos, tp in zip(x_positions, model_tps):
            heights = [
                imec_values.get(name, {}).get((model, tp), float("nan")) for name in tool_names
            ]
            _draw_group(ax, pos, heights, left_limits)
        ax.axhline(1.0, color="#333333", linestyle="--", linewidth=1.0)
    ...
    for idx, concurrency in enumerate(concurrencies):
        ...
        for pos, (inp, out) in zip(x_positions, token_pairs):
            ...
            _draw_group(ax, pos, heights, right_limits)
        ax.axhline(1.0, color="#333333", linestyle="--", linewidth=1.0)
...
if __name__ == "__main__":
    ...
    if h100_imec_rows and h100_nvidia_rows and args.plot in ("ratio", "both"):
        out_dir = Path(PROJECT_ROOT) / "output" / "validation" / "inf"
        genz_h100_rows = None
        genz_h100_nvidia_rows = None
        if COMPARE_GENZ:
            genz_h100_rows = _load_genz_imec_errors(GENZ_IMEC_H100)
            genz_h100_nvidia_rows = _load_genz_nvidia_errors(GENZ_NVIDIA_H100, device="H100")
        combined_ratio_h100 = _plot_combined_ratio_grids_h100(
            h100_imec_rows,
            h100_nvidia_rows,
            out_dir,
            genz_rows=genz_h100_rows,
            genz_nvidia_rows=genz_h100_nvidia_rows,
        )

...
Runs RAPID-LLM training-time estimation against rows in:
  validation_scripts/mosaicml_data/h100_80gb_bf16.csv
and compares predicted training time to inferred_total_latency_s.

Ported from the co-author's SC-era harness (validation_scripts/mosiacml_train.py
in the external fork), with the spelling fixed and one intentional behavior
change — the CANONICAL-model adaptation:

  * The default hardware config is the canonical fitted H100 config
    (validation_configs/hardware-config/H100_SXM5_80GB.fitted.yaml:
    core util 0.85, DRAM util 0.8, network util 0.8, 6 us kernel launch)
    instead of the non-fitted H100_SXM5_80GB.yaml.
  * The SC-era harness injected a FullyConnected 50 GB dp fabric with a
    hand-fitted link utilization of 0.8453452918110231. The fabric shape is
    kept exactly as authored (FullyConnected, 50 GB, 5e-6 s latency, dp-only)
    but the utilization is set to the canonical network utilization of 0.80.
    No other hand-fitted constants survive.

All other validation semantics (spec construction, dp = num_gpus / tp mapping,
ZeRO-3 sharding, reference-time inference from the CSV, plotting) are exactly
as authored.
"""
...
DEFAULT_INPUT_CSV = PROJECT_ROOT / "validation_scripts" / "mosaicml_data" / "h100_80gb_bf16.csv"
...
DEFAULT_HW_CONFIG = (
    ...
    / "validation_configs"
    / "hardware-config"
    / "H100_SXM5_80GB.fitted.yaml"
)
DEFAULT_OUTPUT_CSV = PROJECT_ROOT / "output" / "validation" / "train" / "mosaicml_h100_bf16_all.csv"
DEFAULT_PLOT_OUTPUT = PROJECT_ROOT / "output" / "validation" / "train" / "mosaicml_h100_bf16_all.png"
...
def _mosaic_hardware_overrides() -> Dict[str, object]:
    # Fabric shape as authored in the SC-era harness (FullyConnected, 50 GB,
    # 5e-6 s latency, dp-only). util was hand-fitted to 0.8453452918110231
    # there; under the canonical model it is the canonical network util 0.80.
    return {
        "network": {
            "dimensions": [
                {

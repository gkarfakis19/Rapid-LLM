...
if H100_GPU:
    TRAIN_HW_CONFIG = str(
        REPO_ROOT / "validation_scripts" / "validation_configs" / "hardware-config" / "H100_SXM5_80GB_2d.yaml"
    )
    INF_HW_CONFIG = str(
        REPO_ROOT / "validation_scripts" / "validation_configs" / "hardware-config" / "H100_SXM5_80GB_2d.yaml"
    )
    ...
    TRAIN_HW_CONFIG = str(
        REPO_ROOT / "validation_scripts" / "validation_configs" / "hardware-config" / "a100_80GB_2d_gmap_train.yaml"
    )
    INF_HW_CONFIG = str(
        REPO_ROOT / "validation_scripts" / "validation_configs" / "hardware-config" / "a100_80GB_2d_gmap_inf.yaml"
    )
DERATE_CONFIG = str(REPO_ROOT / "validation_scripts" / "validation_configs" / "harness_derates.yaml")
DERATE_DEVICE_TYPE = "H100_SXM5" if H100_GPU else "A100_SXM4"

TRAIN_70B_MODEL_CONFIG = str(
    REPO_ROOT / "validation_scripts" / "validation_configs" / "model-config" / "Llama3.1-70B_2d_train.yaml"
)
# TRAIN_GPT175B_MODEL_CONFIG = "validation_scripts/validation_configs/model-config/GPT_175_B_2d_train.yaml"
TRAIN_GLM45_106B_MODEL_CONFIG = str(REPO_ROOT / "configs" / "model-config" / "GLM_4.5_AIR_106B.yaml")
TRAIN_VIT7B_MODEL_CONFIG = str(
    REPO_ROOT / "configs" / "model-config" / "vit_7b_patch16_dinov3_lvd1689m_train.yaml"
)
INF_70B_MODEL_CONFIG = str(
    REPO_ROOT / "validation_scripts" / "validation_configs" / "model-config" / "Llama3.1-70B_2d_inf.yaml"
)
# INF_GPT175B_MODEL_CONFIG = "validation_scripts/validation_configs/model-config/GPT_175_B_2d_inf.yaml"
INF_GLM45_106B_MODEL_CONFIG = str(REPO_ROOT / "configs" / "model-config" / "GLM_4.5_AIR_106B_inf.yaml")
INF_VIT7B_MODEL_CONFIG = str(
    REPO_ROOT / "configs" / "model-config" / "vit_7b_patch16_dinov3_lvd1689m_inf.yaml"
)

DEFAULT_OUTPUT_ROOT = SCRIPT_DIR / "artifacts"
...
hfval.ASTRA_TMP_ROOT = OUTPUT_ROOT / "tmp_runs"
...
COMBINED_TOPOLOGY_OUTPUT = "2d_test_super_merged_torus_fullyconnected.png"
...
def _parse_bandwidths(raw: str) -> Tuple[int, ...]:
    if not str(raw or "").strip():
        return tuple(int(v) for v in BW_SWEEP_GBPS)
    values: List[int] = []
    ...
    if not values:
        raise ValueError("--bandwidths must include at least one integer.")
    return tuple(sorted(set(values)))


def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="2D topology test harness (training + inference) for RAPID-LLM."
    )
    parser.add_argument(
        "--output-root",
        type=str,
        default="",
        help=f"Optional directory for 2d_test outputs/cache (default {DEFAULT_OUTPUT_ROOT}).",
    )
    parser.add_argument(
        "--bandwidths",
        type=str,
        default="",
        help="Comma-separated DIM0 bandwidth values (GB/s), e.g. 325,750,1500.",
    )
    parser.add_argument(
        "--derate-config",
        type=str,
        default=DERATE_CONFIG,
        help="Path to derate YAML (default harness_derates.yaml).",
    )
    parser.add_argument(
        "--derate-device-type",
        type=str,
        default=DERATE_DEVICE_TYPE,
        help="Device type entry in derate YAML (default current GPU class).",
    )
    parser.add_argument(
        "--workers",
        type=int,
        default=None,
        help="Optional worker count override for case execution.",
    )
...
def _plot_super_merged_speedup(
    ...
    *,
    ...
    if not model_order or not shapes:
        return
    fig_h = max(2.6, 2.0 + 0.16 * len(shapes))
    with plt.rc_context(ieee_rc_params()):
        ...
        if y_limits is not None:
            ...
            ax.set_ylim(0.95, 1.4)
        ax.set_ylabel(ylabel, fontsize=IEEE_AXIS_TITLE_SIZE_PT)
        ax.set_xlabel("2D topology and GPU count", fontsize=IEEE_AXIS_TITLE_SIZE_PT)
        ax.tick_params(axis="both", labelsize=IEEE_FONT_SIZE_PT)
        ax.axhline(1.0, color="#333333", linestyle="--", linewidth=1.2, alpha=0.85, zorder=3)

        legend_cols = min(len(combined_handles), max(1, len(model_order)))
        legend_rows = max(1, math.ceil(len(combined_handles) / legend_cols))
        fig.suptitle(title, y=0.98, fontsize=IEEE_TITLE_SIZE_PT)
        fig.legend(
        ...
        top_margin = 0.76 if legend_rows > 1 else 0.80
        fig.subplots_adjust(top=top_margin, bottom=0.14, left=0.16, right=0.98)
        fig.savefig(output_path, dpi=IEEE_DPI)
        plt.close(fig)


def _plot_dual_topology_super_merged_speedup(
    torus_train_speedup: Dict[Tuple[str, str], float],
    torus_inf_speedup: Dict[Tuple[str, str], float],
    fc_train_speedup: Dict[Tuple[str, str], float],
    fc_inf_speedup: Dict[Tuple[str, str], float],
    *,
    model_order: Sequence[str],
    shapes: Sequence[str],
    output_path: Path,
    title: str = "2D topology runtime speedup vs Mesh",
    y_limits: Optional[Tuple[float, float]] = None,
) -> None:
    if not model_order or not shapes:
        return

    title_fs = IEEE_TITLE_SIZE_PT + 1
    axis_fs = IEEE_AXIS_TITLE_SIZE_PT + 1
    tick_fs = IEEE_FONT_SIZE_PT + 1
    legend_fs = IEEE_FONT_SIZE_PT + 1
    ...
    with plt.rc_context(ieee_rc_params()):
        fig, axes = plt.subplots(
            1,
            2,
            figsize=(IEEE_HALF_COLUMN_WIDTH_IN * 2.0, fig_h),
            sharey=False,
        )
        ...
        panel_specs = [
            ("2D Torus vs Mesh", torus_train_speedup, torus_inf_speedup),
            ("Fully Connected vs Mesh", fc_train_speedup, fc_inf_speedup),
        ]

        prev_hatch_lw = plt.rcParams.get("hatch.linewidth", None)
        plt.rcParams["hatch.linewidth"] = 0.75
        try:
            for panel_idx, (ax, (panel_title, train_speedup, inf_speedup)) in enumerate(zip(axes, panel_specs)):
                for idx, (model_label, mode) in enumerate(combos):
                    if mode == "train":
                        values = [
                            train_speedup.get((model_label, shape))
                ...
                ax.set_xticks(x)
                ax.set_xticklabels([_format_shape_label(s) for s in shapes], rotation=0, ha="center")
                panel_values = list(train_speedup.values()) + list(inf_speedup.values())
                panel_ylim = (
                    y_limits
                    if y_limits is not None
                    else _speedup_ylim(
                        {("panel", str(i)): float(v) for i, v in enumerate(panel_values)},
                        {},
                    )
                )
                ...
                ax.axhline(1.0, color="#333333", linestyle="--", linewidth=1.2, alpha=0.85, zorder=3)
                if panel_idx == 0:
                    ax.set_ylabel("Speedup over Mesh", fontsize=axis_fs)
        finally:
            if prev_hatch_lw is not None:
                plt.rcParams["hatch.linewidth"] = prev_hatch_lw

        model_handles: List[Patch] = []
        ...
        mode_handles: List[Patch] = [
            Patch(facecolor="#d9d9d9", edgecolor="#333333", hatch="", label="Inference"),
            Patch(facecolor="#d9d9d9", edgecolor="#333333", hatch="//", label="Training"),
        ]
        legend_handles = model_handles + mode_handles
        fig.legend(
        ...
        top_margin = 0.78
        fig.supxlabel("2D topology shape", fontsize=axis_fs, y=0.04)
        fig.subplots_adjust(top=top_margin, bottom=0.21, left=0.08, right=0.995, wspace=0.12)
        fig.savefig(output_path, dpi=IEEE_DPI, bbox_inches="tight")
        plt.close(fig)
...
def _run_for_bandwidth(
    *,
    ...
    if merged_shapes:
        ...
        if "Torus2D" in speedup_maps and "FullyConnected" in speedup_maps:
            torus_train, torus_inf = speedup_maps["Torus2D"]
            fc_train, fc_inf = speedup_maps["FullyConnected"]
            _plot_dual_topology_super_merged_speedup(
                torus_train,
                torus_inf,
                fc_train,
                fc_inf,
                model_order=merged_order,
                shapes=merged_shapes,
                output_path=PLOT_DIR / _with_bw_tag(COMBINED_TOPOLOGY_OUTPUT, bw_tag),
                title=f"2D Torus and Fully Connected vs Mesh (BW={bw_label})",
            )
...
def main() -> None:
    global OUTPUT_ROOT, PLOT_DIR, CACHE_PATH, BW_SWEEP_GBPS
    global DERATE_CONFIG, DERATE_DEVICE_TYPE, NUM_WORKERS
    args = _parse_args()
    if str(args.output_root or "").strip():
        OUTPUT_ROOT = Path(args.output_root).expanduser().resolve()
        PLOT_DIR = OUTPUT_ROOT
        CACHE_PATH = OUTPUT_ROOT / "2d_test_cache.json"
    BW_SWEEP_GBPS = _parse_bandwidths(args.bandwidths)
    DERATE_CONFIG = str(Path(args.derate_config).expanduser().resolve())
    DERATE_DEVICE_TYPE = str(args.derate_device_type or "").strip() or DERATE_DEVICE_TYPE

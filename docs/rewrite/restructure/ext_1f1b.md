...
- **`llm_execution.py:596-597`**: flattened MoE still `NotImplementedError` — DESIGN §2 constraint 4 ("make flattened-MoE possible later without violence") is untested, and the FLAT builder's EP-sync path (`pipeline_fine.py` docstring: *"EP sync collectives (dormant)"*) is dead code nobody has run.

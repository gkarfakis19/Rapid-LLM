...
def validate_program(program: Program, *, check_races: bool = True) -> None:
    """Check V1-V5 (fatal) and V6 (warning, when ``check_races``)."""
    ...
    for idx, op in enumerate(ops):
        ...
        if isinstance(op, TransferOp):
            ...
            for dep in op.post_deps:
                if not (0 <= int(dep) < len(ops)):
                    _fail("V1", f"op {op.uid} post_dep {dep} is not a valid uid")
...
def _warn_group_races(program: Program) -> None:
    ...
    ops = program.ops
    # Reverse adjacency: uid -> dep uids (deps, plus transfer wiring:
    # consumer <- transfer <- producer, plus post_deps).
    ...
    def preds(uid: int) -> List[int]:
        ...
        result.extend(int(d) for d in op.post_deps)
